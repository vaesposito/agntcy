...
function GitHubOAuthCallbackInner() {
  ...
  useEffect(() => {
    ...
    const projectId = sessionStorage.getItem("github_oauth_project_id");
    const sessionKey = sessionStorage.getItem("github_oauth_session_key");
    ...
    const exchange = projectId
      ? api.exchangeGitHubCode(projectId, { code, state })
      : api.exchangeGitHubCodeGlobal({ code, state, session_key: sessionKey! });
